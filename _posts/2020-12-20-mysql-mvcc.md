...
![image-20210225233929554](/assets/img/数据案例.png)
...
![image-20210225235444975](/assets/img/mvcc-1.png)

​2. 假设有第二个事务编号为2对该记录的name做出修改，改为lisi在事务2修改该行记录数据时，数据库会对该行加排他锁然后把该行数据拷贝到undolog中，作为 旧记录，即在undolog中有当前行的拷贝副本拷贝完毕后，修改该行name为lisi，并且修改隐藏字段的事务id为当前事务2的id，回滚指针指向拷贝到undolog的副本记录中事务提交后，释放锁

![image-20210313220450629](/assets/img/mvcc-2.png)
...
![image-20210313220337624](/assets/img/mvcc-3.png)
...
![image-20210227183316573](/assets/img/mvcc-4.png)

在上述的例子中，只有事务4修改过该行记录，并在事务2进行快照读前，就提交了事务，所以该行当前数据的undolog如下所示：

![image-20210227183849998](/assets/img/mvcc-5.png)
...
![image-20210227185820394](/assets/img/mvcc-6.png)
